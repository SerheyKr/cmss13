...
/datum/job/command/tank_crew/get_total_positions(latejoin = FALSE)
	//love the person who do not add null checks
	if(SStechtree.trees[TREE_MARINE].get_node(/datum/tech/arc) && SStechtree.trees[TREE_MARINE].get_node(/datum/tech/arc).unlocked)
		return 0
